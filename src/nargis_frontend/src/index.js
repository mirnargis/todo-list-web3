...
import { nargis_backend } from "../../declarations/nargis_backend";
...
appointForm.addEventListener("submit", async (e) => {
  ...
  try {
    const getId = await nargis_backend.addTodo(obj.textarea);
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    const getAll = await nargis_backend.getAllTodo();
...
async function deleteTodo(userId) {
  try {
    ...
    await nargis_backend.deleteTodo(userId);
